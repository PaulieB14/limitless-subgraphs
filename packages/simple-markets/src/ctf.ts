...
export function handleConditionResolution(event: ConditionResolution): void {
  ...
  // Only count as resolved market if there's an exchange-registered market for this condition
...
export function handlePositionSplit(event: PositionSplit): void {
  ...
  let amountUSD = collateralToUSD(event.params.amount, event.params.collateralToken);
...
export function handlePayoutRedemption(event: PayoutRedemption): void {
  ...
  let payoutUSD = collateralToUSD(event.params.payout, event.params.collateralToken);
